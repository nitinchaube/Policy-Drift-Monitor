...
No API key and no source PDFs are needed to run any of the above. Every model response is
cached in `backend/cache/`, and the coverage-criteria text is committed in
`backend/data/extracted/`, so the whole analysis replays offline from a clean clone and
produces identical numbers. To re-run the model stages for real, copy
`backend/.env.example` to `backend/.env` and add a key.
...
```
notebooks/Pipeline_walk.ipynb    the pipeline built one stage at a time, with no AI
                                  at all until section 5, plus three later additions
...
report/report.pdf                 two pages plus a references page
slides/Presentation.pptx          the deck
video/Presentation Recording.mp4  the five-minute walkthrough
resume/                           current resume goes here
```
...
Section 11 covers three things added after the pipeline was first run for real: voting
across extraction runs, round-trip verification, and provider-facing explanations.
...
**Voting alone did not fix a biased extractor.** Five runs, majority vote, and the
plurality still dropped a real qualifying condition, because the model made the same
mistake most of the time. What fixed it was round-trip verification acting as a referee
